...
def store_old_values(sender, instance, **kwargs):
    ...
    Этот сигнал срабатывает ПЕРВЫМ, до других pre_save сигналов
    """
    if instance.pk:
        try:
            old_task = Task.objects.get(pk=instance.pk)
            # Сохраняем старые значения как атрибуты instance
...
def create_system_comments(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if instance.assignee:
            assignee_name = getattr(instance.assignee, 'get_full_name', lambda: str(instance.assignee))()
            comment_text += f" и назначена на {assignee_name}"
        ...
        if hasattr(instance, '_old_status') and instance._old_status != instance.status:
            ...
            old_status_display = dict(instance._meta.get_field('status').choices).get(
            ...
            TaskComment.objects.create(
                task=instance,
                author=changed_by,
                text=f"Статус изменен: {old_status_display} → {new_status_display}",
                is_system=True
            )

        # Переназначение задачи
        if hasattr(instance, '_old_assignee') and instance._old_assignee != instance.assignee:
            # Безопасно получаем полные имена
            old_assignee_name = "Не назначено"
            if instance._old_assignee:
                old_assignee_name = getattr(
                    instance._old_assignee,
                    'get_full_name',
                    lambda: str(instance._old_assignee)
                )()

            new_assignee_name = "Не назначено"
            if instance.assignee:
                new_assignee_name = getattr(
                    instance.assignee,
                    'get_full_name',
                    lambda: str(instance.assignee)
                )()
        ...
        if hasattr(instance, '_old_deadline') and instance._old_deadline != instance.deadline:
            old_deadline = "Не установлен"
            if instance._old_deadline:
                old_deadline = instance._old_deadline.strftime('%d.%m.%Y %H:%M')

            new_deadline = "Не установлен"
            if instance.deadline:
                new_deadline = instance.deadline.strftime('%d.%m.%Y %H:%M')
...
def set_completed_date(sender, instance, created, **kwargs):
    ...
    if not created and instance.status == 'completed' and not instance.completed_at:
        # Используем update чтобы избежать рекурсии сигналов
